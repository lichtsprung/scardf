...
class Res(val jResource: Resource, val model: Model)
  ...
  override def /(pp: PropPath): NodeBag = pp.foldLeft(NodeBag(this)) {
    _ / _
  }
  ...
  def valuesOf(p: Prop): Iterator[Node] = {
    ...
    new RichStmtIterator(jResource listProperties p.jProperty) map {
      _.o
    }
  }
  ...
  def spreadTo(subgraph: Model, covered: MSet[Res]): Unit = {
    ...
    connectedNodes filter {
      ...
      _.asRes.spreadTo(subgraph, covered)
    }
  }
