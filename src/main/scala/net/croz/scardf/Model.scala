...
class Model(val jModel: JModel) extends util.Logging {
  ...
  def regNs(namespaces: Map[String, String]) =
    namespaces foreach {
      e => jModel.setNsPrefix(e._1, e._2)
    }

  def regNs(pvMappings: Pair[String, Vocabulary]*) {
    regNs(Map(pvMappings map {
      p => (p._1, p._2.prefix)
    }: _*))
  }
  ...
  private def newRes(jResource: Resource) = {
    if (jResource.canAs(classOf[RDFList]))
      new RdfList(jResource.as(classOf[RDFList]), this)
    else if (jResource.canAs(classOf[Property]))
      new Prop(jResource.as(classOf[Property]), this)
